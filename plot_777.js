...
function getAxes(canvas) {    
    return {x: d3.scaleLinear().domain([5.2, 10.4]).range([0, canvas.width]), y: d3.scaleLinear().domain([3.5, 4.6]).range([canvas.height, 0])};    
}
...
function addAttributes(groups, axes) {
    ...
    groups
        .attr('cx', mid_x).attr('cy', mid_y).attr('r', '10')
	...
	.transition().duration(duration * 4).ease(d3.easeLinear)        
